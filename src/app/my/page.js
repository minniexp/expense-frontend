...
export default function ReviewPage() {
  ...
  const handleUpdateSelectedTransactions = async () => {
    ...
    try {
      ...
      const response = await fetch(`${backendUrl}/api/transactions/update-many`, {
        ...
        body: JSON.stringify(selectedTransactionData)
      });
        ...
        if (selectedTransactionData.length > 0) {
          try {
            ...
            if (result.failed && result.failed.length > 0) {
              console.error('Some transactions failed to update:', result.failed);
            }
          } catch (updateError) {
  ...
  const updateReturnDocument = async (returnId, transaction, isAdding) => {
    try {
      ...
      const returnResponse = await fetch(`${backendUrl}/api/returns/${returnId}`);
      if (!returnResponse.ok) {
        console.error(`Failed to fetch return ${returnId}`);
        return;
      }
      
      const returnDoc = await returnResponse.json();
      ...
      const updatedReturn = {
        ...returnDoc,
        total: Math.max(0, parseFloat((returnDoc.total + amountChange).toFixed(2)))
      };
      
      // Update transaction IDs arrays
      if (isAdding) {
        // Adding the transaction ID if it's not already in the array
        if (!returnDoc.returnedTransactionIds || !returnDoc.returnedTransactionIds.includes(transaction._id)) {
          updatedReturn.returnedTransactionIds = [
            ...(returnDoc.returnedTransactionIds || []),
            transaction._id
          ];
        }
        ...
        if (transaction.tellerTransactionId) {
          if (!returnDoc.returnedTellerTransactionIds || !returnDoc.returnedTellerTransactionIds.includes(transaction.tellerTransactionId)) {
            updatedReturn.returnedTellerTransactionIds = [
              ...(returnDoc.returnedTellerTransactionIds || []),
        ...
        updatedReturn.returnedTransactionIds = (returnDoc.returnedTransactionIds || [])
          .filter(id => id !== transaction._id);
          
        // Removing the teller transaction ID if it exists but keeping other teller IDs
        if (transaction.tellerTransactionId) {
          updatedReturn.returnedTellerTransactionIds = (returnDoc.returnedTellerTransactionIds || [])
            .filter(id => id !== transaction.tellerTransactionId);
        }
      }
      
      // Update the return document in the database
      const updateReturnResponse = await fetch(`${backendUrl}/api/returns/${returnId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!updateReturnResponse.ok) {
        console.error(`Failed to update return ${returnId}`);
      } else {
