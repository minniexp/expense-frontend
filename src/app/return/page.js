...
export default function ReturnPage() {
  ...
  const directFetchReturns = async () => {
    try {
      ...
      console.log('Returns data from direct fetch:', data);
  ...
  // Load returns when component mounts with error handling
  useEffect(() => {
    const loadReturns = async () => {
      try {
        await fetchReturnDocuments();
      } catch (error) {
        console.error('Failed to load returns on mount:', error);
      }
    };
