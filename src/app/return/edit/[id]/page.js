...
export default function EditReturnPage({ params }) {
  ...
  const userOptions = [
    { id: process.env.NEXT_PUBLIC_MINID, name: 'Min' },
    { id: process.env.NEXT_PUBLIC_DADID, name: 'Dad Account' },
    { id: process.env.NEXT_PUBLIC_MOMID, name: 'Mom Account' },
    { id: process.env.NEXT_PUBLIC_SHAMID, name: 'SHAM'},
    { id: process.env.NEXT_PUBLIC_YEEMID, name: 'YEEM'},
    { id: process.env.NEXT_PUBLIC_FRIENDID, name: 'friend'},
    { id: process.env.NEXT_PUBLIC_AKMID, name: 'AKM'},
    { id: process.env.NEXT_PUBLIC_KATID, name: 'Kat'}
  ];

  // Fetch return and transactions data
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch return data
        const returnData = await fetchReturn(returnId);
        console.log('Return data:', returnData);
        setReturnDoc(returnData);
        
        // Set form data
        setFormData({
          total: returnData.total.toString(),
          description: returnData.description || '',
          lenderUserId: returnData.lenderUserId || '',
          payeeUserId: returnData.payeeUserId || '',
          returnedTransactionIds: returnData.returnedTransactionIds || [],
          paidBackConfirmationPayee: returnData.paidBackConfirmationPayee || false,
          paidBackConfirmationLender: returnData.paidBackConfirmationLender || false
        });
        
        // Set date
        if (returnData.date) {
          setSelectedDate(new Date(returnData.date));
        }
        ...
        if (returnData.returnedTransactionIds && returnData.returnedTransactionIds.length > 0) {
          await fetchRelatedTransactions(returnData.returnedTransactionIds);
        }
      } catch (error) {
        console.error('Error fetching return data:', error);
        alert(`Failed to load return document: ${error.message}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  // Fetch transactions related to this return
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full bg-gray-900 text-white border border-gray-700 rounded-lg">
              <thead className="bg-gray-800">
                <tr>
                  <th className="px-4 py-2 w-12">#</th> {/* New header for row numbers */}
                  <th className="px-4 py-2">Date</th>
                  <th className="px-4 py-2">Description</th>
                  <th className="px-4 py-2">Amount</th>
                  <th className="px-4 py-2">Category</th>
                  <th className="px-4 py-2">Payment Method</th>
                  <th className="px-4 py-2 w-24">Actions</th> {/* New header for actions */}
                </tr>
              </thead>
              <tbody>
                {transactions.map((transaction, index) => (
                  <tr key={transaction._id} className="border-t border-gray-700 hover:bg-gray-700">
                    <td className="px-4 py-2 text-center text-gray-400 font-mono">{index + 1}</td> {/* Row number */}
